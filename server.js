...
function findTerminalByCoordinate(lat, lng, tolerance = 1e-4) { // 許容誤差を1e-4に拡大
...
const LineSchema = new mongoose.Schema({
    ...
    color: String,
    trackType: String, 
    schedule: { type: [Number], default: [] }, // 運行計画 (ターミナルIDの配列)
}, { collection: 'lines' });
...
class ServerVehicle {
    constructor(id, line, data, formationSize = 1, initialCargo = { passenger: 0, freight: 0, destinationTerminalId: null }, health = 100) {
        this.id = id;
        this.lineId = line.id;
        this.ownerId = line.ownerId;
        this.data = data;
        this.formationSize = formationSize; 
        this.capacity = data.capacity * formationSize; 
        ...
        this.health = health; // 健全性
        this.schedule = line.schedule; // 運行計画をラインから継承
    ...
    move(gameDeltaSeconds) {
        ...
        const healthFactor = Math.max(0.1, this.health / 100); 
        const maxSpeedKms = (this.data.maxSpeedKmH * healthFactor) / 3600; 
        const travelDistanceKm = maxSpeedKms * gameDeltaSeconds; 
        
        // 走行距離に応じて健全性を減少させる (1万kmで100%減少)
        const healthDecrease = travelDistanceKm / 10000; 
        ...
        const safetyDistance = this.category === 'air' ? 10.0 : 1.0; 
        let nextStation = null;
        let minDistance = Infinity;
        
        // 次の停車駅を探すロジック (運行計画の有無に関わらず、進行方向にある最も近い駅を探す)
        this.terminals.filter(t => this.category === 'air' || !t.isAirport).forEach(terminal => {
        ...
        this.updateCoordinates();
        this.checkTerminalArrival(safetyDistance); 
    }
    ...
    handleTerminalArrival(terminal) {
        ...
        if (this.cargo.destinationTerminalId === terminal.id) {
            ...
            if (deliveredLoad > 0) {
                ...
                // 健全性が低い場合の収益ペナルティ
                multiplier *= Math.max(0.5, this.health / 100); 
            ...
            if (availableTerminals.length > 0) {
                ...
                if (this.data.type === 'passenger') {
                    ...
                    if (isRailTerminal) {
                        ...
                        // 空港の場合: 容量の一定割合を積み込む
                        loadAmount = Math.min(availableCapacity, this.capacity * 0.5 * (0.8 + Math.random() * 0.4)); 
                    }
                    this.cargo.passenger += Math.round(loadAmount);
                    
                } else if (this.data.type === 'freight') {
                    const availableCapacity = this.capacity - this.cargo.freight; 
                    let loadAmount = 0;
        ...
        if (this.schedule && this.schedule.length > 1) {
            ...
            const firstTerminalId = this.schedule[0];
            const lastTerminalId = this.schedule[this.schedule.length - 1];
            
            // 運行計画の始点に到着し、かつ逆走中の場合
            if (currentTerminalId === firstTerminalId && this.isReversed) {
                this.isReversed = false;
            } 
            // 運行計画の終点に到着し、かつ順走中の場合
            else if (currentTerminalId === lastTerminalId && !this.isReversed) {
                this.isReversed = true;
            }
            // 運行計画の中間点では折り返さない (単純折り返しロジックを上書き)
        } else {
            // 運行計画がない場合は、路線の物理的な端での折り返しを維持
            if (this.positionKm === this.routeLength) {
                this.isReversed = true;
            } else if (this.positionKm === 0) {
                this.isReversed = false;
...
class ServerLineManager {
    constructor(id, ownerId, terminals, coords, cost, lengthKm, color, trackType, schedule = []) {
        this.id = id;
        this.ownerId = ownerId;
        ...
        this.schedule = schedule; // 運行計画 (ターミナルIDの配列)
        this.vehicles = [];
    }
    async addVehicle(vehicleKey, formationSize = 1) { 
        ...
        if (formationSize > (data.maxFormation || 1)) {
            return { success: false, message: `編成数が最大編成数 (${data.maxFormation}) を超えています。` };
        }
        ...
        const newVehicle = new ServerVehicle(vehicleId, this, data, formationSize); 
        ...
        try {
            await VehicleModel.create({
                ...
                health: newVehicle.health, // healthを保存
            });
        } catch (error) {
...
async function loadUserData(userId) {
    ...
    const lineManagers = linesRes.map(row => {
        ...
        const lineTerminals = coords.map(coord => 
            allTerminals.find(t => coordinatesAlmostEqual([t.lat, t.lng], coord, 1e-4))
        ).filter(t => t);
    ...
    vehiclesRes.forEach(row => {
        ...
        if (line) {
            ...
            const vehicle = new ServerVehicle(row.id, line, data, formationSize, row.cargo, row.health || 100); // healthをロード
...
async function processMonthlyFinancials() {
    ...
    for (const user of Object.values(ServerGame.users)) {
        ...
        user.vehicles.forEach(vehicle => {
            ...
            // 健全性が低い車両は、修理費用とは別に、運行効率低下による間接的なコスト増がある
            monthlyMaintenance += baseCost * vehicle.data.purchaseMultiplier * 0.001 * formationFactor; // 固定資産税 (月0.1%)
        });
...
async function handleLoanRequest(userId, amount, termMonths) {
    ...
    // 資産評価 (ランキング計算と同じロジックを使用)
    const vehicles = user.vehicles;
    let totalVehicleAsset = 0;
    vehicles.forEach(v => {
        const data = VehicleData[v.data.dataKey];
        if (!data) return;
        const baseCost = v.category === 'air' ? AIRPLANE_BASE_COST : VEHICLE_BASE_COST;
        totalVehicleAsset += baseCost * v.data.purchaseMultiplier * (v.formationSize || 1);
    });
    
    const totalAsset = user.money + user.totalConstructionCost * 0.7 + totalVehicleAsset * 0.3;
    ...
    const monthlyRepayment = totalRepayment / termMonths;
    ...
    return { success: true, message: `¥${amount.toLocaleString()} の融資が承認されました。月々 ¥${Math.round(monthlyRepayment).toLocaleString()} の返済です。` };
}
...
async function serverSimulationLoop() { 
    ...
    gameTime.setTime(gameTime.getTime() + (deltaTimeMs * ServerGame.globalStats.timeScale * 1000)); // 1000倍速で時間を進める
    ...
    Object.values(ServerGame.users).forEach(user => {
        ...
        user.vehicles.forEach(v => {
            ...
            vehiclePositions.push({
                ...
                formationSize: v.formationSize,
                health: Math.round(v.health) // healthを追加
            });
        });
        ...
        if (user.socketId) {
            io.to(user.socketId).emit('updateUserState', {
                ...
                establishedLines: user.establishedLines.map(line => ({ 
                    ...
                    schedule: line.schedule, // scheduleを追加
                })),
                vehicles: user.vehicles.map(v => ({ id: v.id, data: v.data, formationSize: v.formationSize, health: v.health, lineId: v.lineId })), // healthを追加
            });
        }
    });
...
io.on('connection', (socket) => {
    ...
    socket.on('login', async (data) => {
        ...
        socket.emit('initialState', {
            ...
            vehicles: userState.vehicles.map(v => ({ id: v.id, data: v.data, formationSize: v.formationSize, health: v.health, lineId: v.lineId })), // healthを追加
        ...
        try {
            ...
            if (!isAdminBuilder) {
                socket.emit('updateUserState', { 
                    ...
                    currentLoan: user.currentLoan,
                    monthlyRepayment: user.monthlyRepayment,
                    vehicles: user.vehicles.map(v => ({ id: v.id, data: v.data, formationSize: v.formationSize, health: v.health, lineId: v.lineId })), 
                });
            } else {
            ...
            socket.emit('error', error.message || '駅の建設中にエラーが発生しました。');
        }
    });
    ...
    socket.on('buildLine', async (data) => {
        ...
        try {
            ...
            if (!isValidConstruction) {
                socket.emit('error', '他プレイヤーの施設は路線のノードとして使用できません。');
                return;
            }
            ...
            io.emit('lineBuilt', {
                ...
                trackType: trackType, cost: totalCost, lengthKm: totalLengthKm, schedule: []
            });
    ...
    socket.on('extendLine', async (data) => {
        ...
        let totalExtensionLengthKm = 0;
        let newTerminalsToAdd = [];
        let newCoordsSegments = [];
        
        let lastCoord = [existingTerminal.lat, existingTerminal.lng];

        for (let i = 0; i < newTerminalCoords.length; i++) {
            const currentCoord = newTerminalCoords[i];
            
            const newTerminal = findTerminalByCoordinate(currentCoord[0], currentCoord[1]);
            
            if (!newTerminal || !newTerminal.terminal) {
                socket.emit('error', `延長ノード ${i + 1} のターミナルが見つかりません。`);
                return;
            }
            ...
            if (terminalToAdd.ownerId !== userId && terminalToAdd.ownerId !== 'ADMIN_SHARED') {
                socket.emit('error', `延長ノード ${i + 1} は他プレイヤーの施設です。`);
                return;
            }
            ...
            if (isAirRoute && !terminalToAdd.isAirport) {
                socket.emit('error', `延長ノード ${i + 1} は航空路線に不適です。`);
                return;
            }
            if (!isAirRoute && terminalToAdd.isAirport) {
                socket.emit('error', `延長ノード ${i + 1} は鉄道路線に不適です。`);
                return;
            }
            ...
            let segmentCoords;
            if (isAirRoute) {
                segmentCoords = calculateGreatCirclePath(lastCoord, currentCoord);
            } else {
                segmentCoords = [lastCoord, currentCoord]; 
            }
            
            const { cost: segmentCost, lengthKm: segmentLengthKm } = calculateConstructionCost(lastCoord, currentCoord, lineManager.trackType);
            
            totalExtensionCost += segmentCost;
            totalExtensionLengthKm += segmentLengthKm;
            
            newTerminalsToAdd.push(terminalToAdd);
            newCoordsSegments.push(segmentCoords);
            lastCoord = currentCoord;
        }
        
        if (user.money < totalExtensionCost) {
            socket.emit('error', `資金不足です！延長費用: ¥${totalExtensionCost.toLocaleString()}`);
            return;
        }
        
        // 資金の更新
        user.money -= totalExtensionCost;
        user.totalConstructionCost += totalExtensionCost;
        
        // 路線データの更新
        let newCoords = [...lineManager.coords];
        let newStations = lineManager.stations.slice();
        
        let segmentCoordsFlat = [];
        newCoordsSegments.forEach(seg => {
            // 既存の座標と重複する最初のノードを削除
            if (segmentCoordsFlat.length > 0 || newCoords.length > 0) {
                seg.shift(); 
            }
            segmentCoordsFlat = segmentCoordsFlat.concat(seg);
        });

        if (isExtendStart) {
            // 始点から延長する場合
            newCoords = segmentCoordsFlat.reverse().concat(newCoords);
            newStations = newTerminalsToAdd.reverse().concat(newStations);
        } else {
            // 終点から延長する場合
            newCoords = newCoords.concat(segmentCoordsFlat);
            newStations = newStations.concat(newTerminalsToAdd);
        }
        
        lineManager.coords = newCoords;
        lineManager.stations = newStations;
        lineManager.cost += totalExtensionCost;
        lineManager.lengthKm += totalExtensionLengthKm;
        ...
        // ターミナル接続情報の更新
        const updatePromises = [];
        newTerminalsToAdd.forEach(terminal => {
            terminal.addLine(lineId);
            const Model = terminal.isAirport ? AirportModel : StationModel;
            updatePromises.push(Model.updateOne(
                { id: terminal.id },
        ...
        socket.emit('info', `路線 ${lineId} を延長しました。費用: ¥${totalExtensionCost.toLocaleString()}`);
    });
    ...
    socket.on('buyVehicle', async (data) => {
        ...
        const formationSize = data.formationSize || 1; 
        
        if (line) {
            const result = await line.addVehicle(data.vehicleKey, formationSize); 
            if (result.success) {
                socket.emit('updateUserState', {
                    money: user.money,
                    currentLoan: user.currentLoan,
    ...
    socket.on('dismantleLine', async (data) => {
        ...
        if (result.success) {
            ...
            socket.emit('updateUserState', {
                ...
                currentLoan: user.currentLoan,
                monthlyRepayment: user.monthlyRepayment,
                establishedLines: user.establishedLines.map(l => ({ id: l.id, ownerId: l.ownerId, coords: l.coords, color: l.color, trackType: l.trackType, cost: l.cost, schedule: l.schedule })),
                vehicles: user.vehicles.map(v => ({ id: v.id, data: v.data, formationSize: v.formationSize, health: v.health, lineId: v.lineId })),
            });
            socket.emit('info', result.message);
        } else {
    ...
    socket.on('setLineSchedule', async (data) => {
        if (!userId) return;
        const { lineId, terminalIds } = data;
        ...
        // 運行計画のバリデーション (路線上のターミナルIDのみを含むか確認)
        const validTerminalIds = line.stations.map(s => s.id);
        const isValidSchedule = terminalIds.every(id => validTerminalIds.includes(id));
        
        if (!isValidSchedule) {
            socket.emit('error', '運行計画には、その路線上のターミナルIDのみを含める必要があります。');
            return;
        }
        
        line.schedule = terminalIds;
        
        // DBに保存
        await LineModel.updateOne({ id: lineId }, { $set: { schedule: terminalIds } });
        
        // 車両にも反映
        line.vehicles.forEach(v => {
            v.schedule = terminalIds;
        });
        ...
        socket.emit('info', `路線 L${lineId} の運行計画を更新しました。`);
    });
    
    socket.on('repairVehicle', async (data) => {
        if (!userId) return;
        ...
        if (!vehicle) {
            socket.emit('error', '車両が見つかりません。');
            return;
        }
        
        if (vehicle.health >= 100) {
            socket.emit('info', 'この車両は修理の必要がありません。');
            return;
        }
        
        const repairCost = Math.round((100 - vehicle.health) * 100000); // 健全性1%あたり10万円
        
        if (user.money < repairCost) {
            socket.emit('error', `修理費用が不足しています。(¥${repairCost.toLocaleString()})`);
            return;
        }
        ...
        // DBに保存
        await VehicleModel.updateOne({ id: vehicleId }, { $set: { health: 100, status: vehicle.status } });
        
        user.moneyUpdated = true;
        socket.emit('info', `車両 #${vehicleId} を修理しました。費用: ¥${repairCost.toLocaleString()}`);
    });
